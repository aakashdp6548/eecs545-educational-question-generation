...
class ModelHandler:
    ...
    def _build_model(self, checkpoint=None):
        if checkpoint:
            print(f'Loading previous checkpoint from {checkpoint}')
            self.model = torch.load(checkpoint).to(self.config['device'])
        else:
            ...
            for i, data in enumerate(self.train_dataloader):
                ...
                if self.use_copy:
                    oov_dict, tgt = prepare_ext_vocab(
                        data["graph_data"],
                        self.vocab,
                        gt_str=data["tgt_text"],  # output_str instead of tgt_text for Text2TextDataset
    ...
    def evaluate(self, dataloader):
        ...
        with torch.no_grad():
            ...
            for data in dataloader:
                ...
                gt_collect.extend(data["tgt_text"])  # output_str instead of tgt_text for Text2TextDataset
...
def main(config, checkpoint):
    ...
    # save_model_checkpoint(runner.model, 'saved_model/params.saved')
...
if __name__ == "__main__":
    ...
    if cfg["grid_search"]:
        ...
        main(g2s_template, cfg['checkpoint'])
